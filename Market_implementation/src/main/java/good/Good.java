...
public class Good implements Comparable{
    ...
    private static int retrieveLatestId(){
        ...
        try(SQLConnector connector = new SQLConnector()){
            ResultSet resultSet = connector.runQuery(good.SQLQueries.GET_LATEST_ID, AppProperties.getProperty(PropertiesLabels.getMarketDatabase()));
...
//        float ceiling = price * ((100+PRICE_VARIANCE_PERCENT)/100f);
    ...
    private static float getAvgMarketPrice(){
        float price = 100;
        try(SQLConnector connector = new SQLConnector()){
            ResultSet resultSet = connector.runQuery(SQLQueries.GET_AVERAGE_MARKET_PRICE,AppProperties.getProperty(PropertiesLabels.getMarketDatabase()));
            while(resultSet.next()){
    ...
    private int findId(){
        int idToReturn = 0;
        try(SQLConnector connector = new SQLConnector()){
            ResultSet resultSet = connector.runQuery(SQLQueries.GET_HIGHEST_ID,AppProperties.getProperty(PropertiesLabels.getMarketDatabase()));
            while(resultSet.next()){
